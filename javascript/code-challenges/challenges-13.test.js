...
// Solution code here...
return arr.indexOf(
  arr.reduce((acc, item, index) => (acc.length > item.length ? acc : item), '')
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter((item) => (item.includes(':)') ? item : null));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((item, index) => (index % 2 !== 0 ? item : null)).join('');
};
