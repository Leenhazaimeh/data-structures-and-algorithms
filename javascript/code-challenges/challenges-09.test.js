...
const maxInArray = (arr) => {
  // Solution code here...
  const valueArray = arr.reduce((acc,value) => Math.max(acc, value));
  return valueArray;
};
...
const checkValues = (obj, value) => {
  // Solution code here...
  const newObject = Object.values(obj).includes(value);
  return newObject;
};
...
const updateNumbers = (obj) => {
  // Solution code here...
  const newObject = Object.entries(obj).map (element=> element.join(': '));
  return newObject;
};
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  Object.values(arr).map(element => houses.push(element.house));
  return houses;
...
const hasChildrenValues = (arr, character) => {
  ...
  for(const items of arr){
    if(Object.values(items).includes (character)){
